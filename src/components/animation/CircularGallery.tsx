...
class Title {
	...
	constructor({ gl, plane, renderer }: TitleProps) {
		autoBind(this);
		this.gl = gl;
		this.plane = plane;
...
class Media {
	...
	onResize({
		...
		this.plane.scale.y =
			(this.viewport.height * (250 * this.scale)) / this.screen.height;
		this.plane.scale.x =
			(this.viewport.width * (375 * this.scale)) / this.screen.width;
...
class App {
	...
	constructor(
		container: HTMLElement,
		{ items, bend = 1, borderRadius = 0 }: AppConfig
	) {
